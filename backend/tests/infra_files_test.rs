...
fn docker_compose_contains_postgres_service() {
    ...
    assert!(
        ...
        "compose file must define a postgres service"
    );
}
...
fn docker_compose_references_named_volume() {
    ...
    assert!(
        ...
        "compose file must declare a named volume"
    );
}
...
fn docker_compose_exposes_postgres_port() {
    ...
    assert!(
        ...
        "compose file must expose postgres port 5432"
    );
}
...
fn root_env_example_documents_postgres_vars() {
    ...
    assert!(
        ...
        "root .env.example must document POSTGRES_USER"
    );
    assert!(
        content.contains("POSTGRES_PASSWORD"),
        "root .env.example must document POSTGRES_PASSWORD"
    );
    assert!(
        content.contains("POSTGRES_DB"),
        "root .env.example must document POSTGRES_DB"
    );
}
...
fn backend_env_example_documents_database_url() {
    ...
    assert!(
        ...
        "backend/.env.example must document DATABASE_URL"
    );
}
