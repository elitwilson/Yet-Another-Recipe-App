...
// Run with: DATABASE_URL=postgres://yara:yara@localhost:5432/yara cargo test --test recipes
...
async fn get_api_recipes_body_is_array_with_seeded_rows() {
    ...
    assert!(!arr.is_empty(), "seeded DB must return at least one recipe");
    let first = &arr[0];
    assert!(
    ...
    assert!(
        ...
        "each recipe must have a name field"
    );
